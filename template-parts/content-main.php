<div class="col-md-6 my-auto pr-5">
    <h2>Our Consultancy</h2>
    <h5>Our Experience</h5>
    <p>Hunter Burton Inc. was incorporated in 1994 by Dr. Charles and Portia Hunter serving as the entity for our first business. After a combined 50 years of corporate and business ownership experience, we decided to combine our knowledge and passion for business operations, project management, and compliance to helping others optimize current operations and build strong and relevant brand.</p>
    <p>Our process is designed to empower you and your team, to outfit them with the tools they need to succeed. Talk to us today about how we can support your industry compliance, policy and procedure documentation, and staff process training.  In addition to personal and business strategy. </p>
    <p>Look to us as your THOUGHT PARTNERS.</p>
    
    <h5>Our Approach</h5>
    <p>Our team gains understanding of the client’s driving vision and passion for their current or new business, consultancy, or non-profit. We identify high priority themes to formulate a feasible and actionable strategy platform.</p>
    <p>We provide our client with consistent and innovatively strategic support in the areas of planning, positioning, and comprehensive marketing strategy based on the vision.</p>
    <p>Next, we present an actionable roadmap for the client to follow to execute the designed strategy.</p>

    <h5>Why Us?</h5>
    <p>Business mentors are key—that’s why when it comes to client selection, we’re choosy. We want to give each of you the time and guidance you deserve.  Whether you’re seeking a strategic alliance with the right partner or a special skillset or tool, call us today. Together we’ll create and refine your plan for success. We didn’t get there alone. And neither will you. </p>
</div>
<div class="col-md-6">
    <img src="<?php echo get_template_directory_uri(); ?>/assets/main.png" width="759">
</div>